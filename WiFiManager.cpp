...
// this fixes most ap problems, however, simply doing mode(WIFI_AP) does not work if sta connection is hanging, must `wifi_station_disconnect`
  ...
  // Count networks per channel to find the least congested one
  int channelCount[14] = {0}; // WiFi channels 1-13 (index 0 unused)
  ...
  for (int i = 0; i < WiFi.scanComplete(); i++)
  {
    ...
    // Count networks on each channel (channels 1-13)
    if (networkChannel >= 1 && networkChannel <= 13)
    {
      channelCount[networkChannel]++;
    }
  }

  // Find channel with least networks
  ...
  // Update channel to the one with least congestion
  channel = bestChannel;

  setWiFiAPChannel(channel);

  DEBUG_WM(WM_DEBUG_NOTIFY, F("Using channel: "), channel);
